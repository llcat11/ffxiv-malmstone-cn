        ...
        private void ShowSeriesProgressionToast(AddonEvent eventType, AddonArgs addonInfo)
        {
            ...
            switch (Plugin.Configuration.PostmatchProgressionToastType)
            {
                case 0:
                    ...
                    break;
                case 1:
                    Plugin.ToastGui.ShowQuest("系列赛等级 " + CurrentSeriesLevel +
                        "     " + seriesInfo.SeriesExperience + "/" + MalmstoneXPCalculator.GetXPTargetForCurrentLevel(seriesInfo.CurrentSeriesRank));
                    break;
                case 2:
                    Plugin.ToastGui.ShowError("系列赛等级 " + CurrentSeriesLevel +
                        "     " + seriesInfo.SeriesExperience + "/" + MalmstoneXPCalculator.GetXPTargetForCurrentLevel(seriesInfo.CurrentSeriesRank));
                    break;
                default:
                    Plugin.ToastGui.ShowNormal("系列赛等级 " + CurrentSeriesLevel +
        ...
        private void ShowSeriesProgressionMessage(PvPSeriesInfo seriesInfo, PvPContentType contentType)
        {
            ...
            switch (contentType)
            {
                ...
                case PvPContentType.Frontlines:
                    ...
                    seString.Append(new TextPayload($"第 2 名：{flResultData.FrontlineLose2nd} " + (flResultData.FrontlineLose2nd == 1 ? "场" : "场") + " （" + (flResultData.FrontlineDailyLose2nd) + "）\n"));
